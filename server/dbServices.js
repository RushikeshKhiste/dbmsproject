...
class Dbservices{
   ...
   async getAllData(){
    ...
    async searchHostelsById(id){
        try{
            const response = await new Promise((resolve,reject)=>{
                const query = 'select h_name,h_addr,pincode,h_facilities,h_fees,avg(h_rating) as avg_rating,h_review from hostelTable where pincode = (select pincode from collegeTable where c_id = ?) group by h_name order by avg_rating desc;';
